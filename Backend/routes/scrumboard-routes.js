...
router.get('/project_task', (req, res) => {
    const query = 'SELECT id, name FROM project_task'; // ✅ Include id for dropdown value
...
router.get('/columns-with-tasks', (req, res) => {
    const query = `
        ...
        LEFT JOIN project_task p ON t.project_id = p.id
        ORDER BY ps.id, t.id
    `;
