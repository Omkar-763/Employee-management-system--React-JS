...
import IconDelete from '../../components/Icon/IconBell';
...
const ProjectManagement: React.FC = () => {
    ...
    const filterProjects = () => {
        let filtered = projects.filter((project) => (selectedTab === 'active' ? !project.isDeleted : project.isDeleted));
    ...
    const handleCreateProject = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorMessage = data.error || data.message || (data.errors ? JSON.stringify(data.errors) : 'Failed to create project');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    setProjects((prev) => prev.map((project) => (project.id === projectId ? { ...project, isDeleted: true, deletedAt: new Date().toISOString() } : project)));
    ...
    const handleRestoreProject = async (projectId: string) => {
        try {
            ...
            setProjects((prev) => prev.map((project) => (project.id === projectId ? { ...project, isDeleted: false, updatedAt: new Date().toISOString() } : project)));
    ...
    return (
        <div className="flex gap-5 relative sm:h-[calc(100vh_-_150px)] h-full">
                        ...
                        <div className="space-y-1">
                            ...
                            >
                                ...
                                <div className="bg-primary-light dark:bg-[#060818] rounded-md py-0.5 px-2 font-semibold whitespace-nowrap">
                                    {projects.filter((p) => p && p.isDeleted === false).length}
                                </div>
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                ...
                                <div className="bg-primary-light dark:bg-[#060818] rounded-md py-0.5 px-2 font-semibold whitespace-nowrap">{projects.filter((p) => p.isDeleted).length}</div>
                            </button>
                        </div>
                    </PerfectScrollbar>
                </div>
            ...
            {/* Main content */}
            <div
                className={`overlay bg-black/60 z-[5] w-full h-full rounded-md absolute hidden ${isShowProjectMenu && '!block xl:!hidden'}`}
                onClick={() => setIsShowProjectMenu(!isShowProjectMenu)}
            ></div>
            <div className="panel p-0 flex-1 overflow-auto h-full">
                <div className="flex flex-col h-full">
                            ...
                            <form onSubmit={handleCreateProject} className="space-y-5">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input id="startDate" type="date" className="form-input" value={newProjectStartDate} onChange={(e) => setNewProjectStartDate(e.target.value)} required />
                                    </div>
                                    <div>
                                ...
                                <div className="flex justify-end space-x-4">
                                    <button type="submit" className="btn btn-primary flex items-center" disabled={isLoading || !newProjectName.trim() || !newProjectStartDate || !newProjectEndDate}>
                    ...
                    <div className="p-4">
                        <h3 className="text-lg font-semibold mb-4">{selectedTab === 'active' ? 'Active Projects' : 'Recycle Bin'}</h3>
                                    ...
                                    {isLoading ? (
                                        ...
                                        filteredProjects.map((project) => (
                                            <tr key={project.id}>
                                                ...
                                                {selectedTab === 'active' && <td>{getProjectStatus(project.startDate, project.endDate)}</td>}
                                                <td>{formatDate(project.startDate)}</td>
                                                <td>{formatDate(project.endDate)}</td>
                                                <td>{formatDateTime(project.createdAt)}</td>
                                                {selectedTab === 'recycleBin' && <td>{project.deletedAt ? formatDateTime(project.deletedAt) : 'N/A'}</td>}
                                                <td>
                                                    <div className="flex items-center space-x-2">
                                                        {selectedTab === 'active' ? (
                                                            ...
                                                            <>
                                                                <button type="button" className="btn btn-outline-success p-2" onClick={() => handleRestoreProject(project.id)} title="Restore Project">
                                        ...
                                        <tr>
                                            <td colSpan={isAdmin ? (selectedTab === 'recycleBin' ? 7 : 6) : selectedTab === 'recycleBin' ? 6 : 5} className="text-center py-4">
                                                {selectedTab === 'active' ? 'No active projects found. Create your first project!' : 'Recycle bin is empty'}
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
                        ...
                        <form onSubmit={handleUpdateProject} className="space-y-4">
                            <div>
                                ...
                                <input id="editProjectName" type="text" className="form-input" value={editProjectName} onChange={(e) => setEditProjectName(e.target.value)} required />
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label htmlFor="editStartDate">Start Date</label>
                                    <input id="editStartDate" type="date" className="form-input" value={editProjectStartDate} onChange={(e) => setEditProjectStartDate(e.target.value)} required />
                                </div>
                                <div>
                            ...
                            <div className="flex justify-end space-x-3">
                                <button type="button" className="btn btn-outline-danger" onClick={() => setEditingProject(null)}>
                                    Cancel
                                </button>
